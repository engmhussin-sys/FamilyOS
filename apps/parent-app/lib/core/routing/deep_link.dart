/// THE CLIENT HALF OF `abny://` — string in, typed destination out, and it
/// never throws.
///
/// WHAT WAS BROKEN. Every notification this product has ever sent ended in a
/// tap that did nothing: `notifications_screen.dart`'s row `onTap` marked the
/// row read and stopped there. The server side of the gap is closed —
/// `apps/backend/src/modules/notifications/domain/engine/notification-destination.ts`
/// now resolves a destination for EVERY notification and delivers it on the
/// row's `data` payload under the key `deepLink`. This file is the client's
/// side of that same contract, and nothing else in this app is allowed to
/// decide where a notification leads.
///
/// THE SERVER IS AUTHORITATIVE. This file parses; it does not infer. It never
/// looks at `type`, never reads the Arabic body, and never guesses a screen
/// from a notification's shape — a client that guessed would be a second
/// opinion nobody can audit, and it would drift from the server inside a
/// sprint.
///
/// ---------------------------------------------------------------------------
/// THE SCHEME, canonical and fixed: `abny://<surface>[/<id>]`. No query string,
/// no fragment, no port, no userinfo, no absolute URL, and never a token. It is
...
/// links the producer would call invalid, and a validator that is *stricter*
/// rejects links the producer calls valid. Neither is a contract.
///
/// ---------------------------------------------------------------------------
/// TOTALITY, AND WHAT DEGRADES TO THE INBOX. [parseDeepLink] has no failure
/// mode that is not a value: `null`, an empty or blank string, a wrong scheme
/// (`https://`, `abny:/goals`, `ABNY://goals`), an unknown surface, an id whose
/// shape is not an id, a MISSING id on a surface that requires one, an EXTRA
/// segment on a surface that takes none — every one of them returns
/// [DeepLinkDestination.inbox]. The inbox is where the notification itself is,
/// so it is always a truthful answer, and it is never an exception on a path a
/// user reached by tapping something.
///
/// WHY A MISSING ID IS THE INBOX AND NOT THE LIST. The server never emits a
/// bare id-bearing surface: `notification-destination.ts` already degrades
/// `goal` to `goals`, `approval` to `approvals` and `safety` to `screen-time`
/// itself, at the point where it knows whether an id exists. So `abny://goal`
/// arriving here does not mean "the goal list" — it means this client and that
/// server disagree about the contract, and the honest landing for a broken
/// contract is the inbox rather than a screen we picked on the server's behalf.
///
...
/// no path separators, no `?`/`#`, no whitespace, no `.`/`..`, and bounded in
/// length. AND IT IS NEVER AN AUTHORIZATION CLAIM: an id here says only which
/// row the next screen will ASK for. The server decides whether this parent may
/// have it, on that next call, exactly as it does for a row reached by tapping
/// a list.
library;
...
class DeepLinkDestination {
  ...
  /// Non-null exactly when [surface] takes an id and a well-shaped one arrived.
  final String? id;
...
/// THE ONE ENTRY POINT, AND IT IS TOTAL. Every rejection is
/// [DeepLinkDestination.inbox]; there is no path out of here that throws and no
/// path that returns null.
DeepLinkDestination parseDeepLink(String? link) {
  ...
  if (!deepLinkSurfaceTakesId(surface)) {
    ...
    // understand, and pretending otherwise would route a user somewhere the
    // server did not send them.
    return id == null ? DeepLinkDestination(surface) : DeepLinkDestination.inbox;
  }

  if (id == null) return DeepLinkDestination.inbox;
...
/// Pulls the link off ONE notification row as the parent app receives it from
/// `GET /notifications` — `{ id, title, body, data: { deepLink }, readAt }` —
/// and, equally, off a raw FCM `data` map where the key sits at the top level.
/// Both shapes carry the same key; accepting both means one tap handler serves
/// an inbox row and a cold-start push payload without a second parser.
///
