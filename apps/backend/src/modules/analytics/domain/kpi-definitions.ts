...
export const KPI_DEFINITIONS: Readonly<Record<KpiId, KpiDefinition>> = {
  ...
  STICKINESS: {
    ...
    kind: 'RATE',
    formula: 'DAU / MAU — both counted by the same function, so they cannot drift',
    numerator: 'DAU, as counted above',
    denominator: 'MAU, as counted above',
    windowDays: 30,
  ...
  ARR: {
    ...
    kind: 'MONEY_MINOR',
    formula: 'MRR × 12 — derived from MRR, never summed independently',
    numerator: 'MRR, as computed above',
    denominator: null,
    windowDays: null,
  ...
  LTV_CAC_RATIO: {
    ...
    kind: 'RATIO',
    formula: 'LTV / CAC, both in the SAME currency (a mixed-currency ratio throws)',
    numerator: 'LTV, tagged FORECAST',
    denominator: 'CAC, tagged ACTUAL',
    windowDays: null,
    source: 'derived',
  ...
  PAYBACK_MONTHS: {
    ...
    numerator: 'CAC, as computed above',
